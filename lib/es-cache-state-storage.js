...
class EsCacheStateStorage extends CacheStateStorage {
    constructor(context, config) {
        ...
        this.persist_field = config.persist_field || config.id_field;
    ...
    _dedupeDocs(docArray, dedupField = this.id_field) {
        // returns uniq docs from an array of docs
        const uniqKeys = {};
        return docArray.filter((doc) => {
            const id = doc[dedupField];
    ...
    async mset(docArray) {
        const dedupedDocs = this._dedupeDocs(docArray, this.persist_field);
        if (this.persist) {
